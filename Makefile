...
DOCKER_TAG?=jaytwo_timerounding
...
build: restore
	dotnet build ./jaytwo.TimeRounding.sln
...
unit-test: build
	...
	rm -rf out/coverage
	cd ./test/jaytwo.TimeRounding.Tests; \
		dotnet test \
		--results-directory ../../out/testResults \
		--logger "trx;LogFileName=jaytwo.TimeRounding.Tests.trx"
	reportgenerator \
